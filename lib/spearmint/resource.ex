...
defmodule Spearmint.Resource do
  @moduledoc """
  ...
  Resources are defined by invoking `use Spearmint.Resource` in their module definition.
  ...
  1. At compile time, when invoking the `use Spearmint.Resource`, by providing the `:state` option.
    ```elixir
    defmodule Demo.Resources.PlayerCount do
      use Spearmint.Resource, state: [player_count: 0, max_players: 100]
    end
    ```

  2. At runtime when creating the resources from specs: `t:Spearmint.Resource.resource_spec()`
    ```elixir
  ...
  **Optional** callback to validate the resource state.
  ...
  See `c:Spearmint.Component.validate/1` for more details.
  """
  @callback validate(resource :: struct()) :: :ok | {:error, any()}
  @optional_callbacks validate: 1

  @doc """
  Fetches the resource. It has the same functionality as `Spearmint.Query.fetch_resource/1`,
  ...
  ## Examples:
    ...
    ``` elixir
      ...
      {:ok, %Demo.Resources.Config{} = resource} = Spearmint.Query.fetch_resource(Demo.Resources.Config)
    ```
  """
  ...
  def debug do
    ...
    :ets.match_object(Spearmint.Util.resources_state_ets_table(), {:"$0", :"$1", :"$2"})
  end
  ...
  defmacro __using__(opts) do
    quote bind_quoted: [opts: opts], location: :keep do
      ...
      def fetch do
        Spearmint.Query.fetch_resource(__MODULE__)
      end
    end
  end
end
