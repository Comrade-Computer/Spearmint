...
defmodule Spearmint.Template.Component.Timer do
  @moduledoc """
  The `Timer` is a **Template Component** designed to facilitate the creation
  of custom timer (countdown) components.

  It serves as a foundation for building custom timer components with `use Spearmint.Template.Component.Timer`.
  ...
  A dedicated `Spearmint.System.Timer` system is provided by the framework.
  This system auto-decrements the Timer component's time
  and dispatches an event when time reaches 0.
  To ensure functionality, this System must be manually included in the `c:Spearmint.setup/1`.
  ...
  - `:duration` - the timer's duration in frames which also serves
  as the reset value.
  - `:time` - the current time of the timer in frames,
  auto-decremented by the framework each frame.
  - `:event` - the event module dispatched when timer reaches 0.
    - create special timer events using `Spearmint.Template.Event.Timer`.
  ...
  ## Example:
    ...
    defmodule Demo.Events.RestoreEnergy do
      use Spearmint.Template.Template.Event.Timer
    end
  end
  ```

  """
  use Spearmint.Template.Component,
    tags: [:ecs_timer],
    state: [:duration, :time, :event, mode: :repeat, paused: false]

  @mode [:repeat, :once, :temporary]

  # This allows the template validate function to be called in the runtime validation as well
  @impl Spearmint.Template.Component
  def validate(state) when is_list(state) do
    validate(Map.new(state))
  end
  ...
  @doc false
  @spec timer_component_tag() :: :ecs_timer
