...
defmodule Spearmint.Template.Component do
  @moduledoc """
  ...
  See `Spearmint.Component` for more details.
  ...
  - `Spearmint.Template.Component.Timer` - a component template that is used to create timer components.

  ## Options

  See `Spearmint.Component` for the list of options.

  ## Examples

    ```elixir
      defmodule Demo.Componenets.Resource do
        use Spearmint.Template.Component, state: [amount: 0], tags: [:resource]
      end
  ...
  **Optional** callback to validate the template and component state fields.
  ...
  For runtime component state validation see `c:Spearmint.Component.validate/1`.

  ## Examples

    ```elixir
    defmodule Demo.Components.Resources do
      use Spearmint.Template.Component, state: [amount: 0]
  ...
  defmacro __using__(opts) do
    ...
    quote bind_quoted: [opts: opts], location: :keep do
      @behaviour Spearmint.Template.Component
      ...
      defmacro __using__(opts) do
        quote bind_quoted: [
          ...
          use Spearmint.Component, state: state, tags: tags
        end
      end
    end
  end
