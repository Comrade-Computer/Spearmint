...
defmodule Spearmint.Projection do
  @moduledoc """
  The `Spearmint.Projection` behaviour is used to build state projections.
  The projections are defined by invoking `use Spearmint.Projection` in their module definition.
  ...
  The module invoking `use Spearmint.Projection` must implement the mandatory
  `c:Spearmint.Projection.project/1` callback. This is responsible for
  ...
  > #### Note  {: .warning}
  > The `project/1` callback runs every frame, after executing all systems.
  ...
  ### The Projection
    ...
    defmodule Demo.Projections.Hero do
      use Spearmint.Projection, fields: [:pos_x, :pos_y, :resources_gold, :resources_gems]
  ...
  > #### Info  {: .info}
  > The `attrs` map is passed to the `c:Spearmint.Projection.project/1`
  > and `c:Spearmint.Projection.on_change/3` callbacks.
  ...
  It takes the `attrs` map argument passed to `c:Spearmint.Projection.start!/1`.
  ...
  ## Examples
    ...
    ```elixir
    %Spearmint.Projection{state: :ok, result: %Demo.Projection.Hero{}} =
  ...
  defmacro __using__(opts) do
    ...
    quote bind_quoted: [opts: opts], location: :keep do
      ...
      @behaviour Spearmint.Projection
      ...
      def start!(attrs \\ %{}) do
        ...
        {:ok, pid} =
          DynamicSupervisor.start_child(
            ...
            {Spearmint.Projection.Server, %{attrs: attrs, projection_module: __MODULE__}}
          )
      ...
      def stop(projection_pid) do
        ...
        DynamicSupervisor.terminate_child(Spearmint.Projection.Supervisor, projection_pid)
      end
    end
  end
end
