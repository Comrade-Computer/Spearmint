...
defmodule Spearmint.Command do
  @moduledoc """
  The `Spearmint.Command` module provides a set of functions for managing entities, components and resources in the `Spearmint` engine.

  Commands are the only way to change the state of components and resources in `Spearmint`. These commands can only be run from systems, otherwise an error will be thrown.
  The `Spearmint.Command` module includes functions for managing relationships between entities, such as adding and removing children and parents.
  ...
  The `Spearmint.Command` module provides functions for managing relationships between entities.
  This is aslo a powerful tool to manage different kind of collections.

  > #### Spearmint entities relationships are **bidirectional associations** {: .info}
  ...
  # TODO Many of the Commands are for fetching from the singleton. Many will need to be updated to pull from the target pid's ets instead of named ets

  alias __MODULE__
  alias Spearmint.Component
  alias Spearmint.Entity
  alias Spearmint.Query
  alias Spearmint.Resource
  ...
  defmodule Operation do
    ...
    @type t :: %__MODULE__{
            name: name(),
            system: module(),
            entities_components: %{(entity_id :: binary()) => list(component_module :: module())},
  ...
  defmodule Error do
    ...
    @spec exception({Spearmint.Command.Operation.t(), binary()}) :: map()
    def exception({%Spearmint.Command.Operation{} = operation, message}) do
      msg = """
  ...
  @doc """
  Spawns a new entity with the given components and relations provided by the Spearmint.Entity.entity_spec() type.
  ...
  ## Examples
    ...
    ```elixir
      %Spearmint.Entity{} = Spearmint.Command.spawn_entity!(
        {
          Spearmint.Entity,
          id: "my_custom_id",
  ...
  It takes a list of entity specs as argument and returns a list of Spearmint.Entity structs.
  ...
  ## Examples
    ...
    ```elixir
    :ok = Spearmint.Command.despawn_entity!(hero_entity)
    ```
  """
  @doc group: :entities
  ...
  > #### Note  {: .info}
  > The entity's `Spearmint.Component.Children` and `Spearmint.Component.Parents` components are not cloned.
  ...
  ## Examples
    ...
    ```elixir
    %Spearmint.Entity{} = entity = Spearmint.Command.clone_entity!(compass_entity)
    ```
  """
  @doc group: :entities
  @spec clone_entity!(Entity.t(), opts :: keyword()) :: Entity.t()
  def clone_entity!(entity, opts \\ []) do
    components = Spearmint.Query.list_components(entity)

    component_specs =
      Enum.map(components, fn component ->
        state = component |> Map.from_struct() |> Map.delete(:__meta__) |> Map.to_list()
        {component.__struct__, state, Spearmint.Query.list_tags(component)}
      end)
    ...
    case Keyword.fetch(opts, :id) do
      {:ok, entity_id} when is_binary(entity_id) ->
        spawn_entity!(
          {Spearmint.Entity,
           id: entity_id, components: component_specs, children: [], parents: []}
        )

      _else ->
        spawn_entity!({Spearmint.Entity, components: component_specs, children: [], parents: []})
    end
  end
  ...
  ## Examples
    ...
    ```elixir
    %Spearmint.Entity{} = entity = Spearmint.Command.deep_clone_entity!(enemy_entity)
    ```
  """
  @doc group: :entities
  ...
  def deep_clone_entity!(entity, opts \\ []) do
    ...
    children = Spearmint.Query.list_children(entity)
  ...
  ## Examples
    ...
    ```elixir
    :ok = Spearmint.Command.add_component!(hero_entity, Demo.Components.Gold)
    :ok = Spearmint.Command.add_component!(hero_entity, {Demo.Components.Gold, [amount: 5], [:resource, :available]})
    ```
  """
  ...
  def add_and_fetch_component!(entity, component_spec) do
    ...
    Spearmint.Query.fetch_component(entity, component_module)
  end
  ...
  ## Examples
    ```elixir
    :ok = Spearmint.Command.add_components!([
  ...
  ## Examples
    ...
    ```elixir
    :ok = Spearmint.Command.update_component!(position_component, x: :12)
    ```
  """
  @doc group: :components
  ...
  def update_and_fetch_component!(component, changes_keyword) do
    ...
    Spearmint.Query.fetch_component(component_meta.entity, component_meta.module)
  end
  ...
  ## Examples
    ...
    ```elixir
    :ok = Spearmint.Command.update_components!([
  ...
  ## Examples
    ...
    ```elixir
    :ok = Spearmint.Command.remove_component!(invisibility_component)
    ```
  """
  @doc group: :components
  ...
  ## Examples
    ...
    ```elixir
    :ok = Spearmint.Command.add_child!(hero_entity, sword_entity)
    ```
  """
  ...
  ## Examples
    ...
    ```elixir
    :ok = Spearmint.Command.add_children!([
  ...
  ## Examples
    ...
    ```elixir
    :ok = Spearmint.Command.add_parent!(sword_entity, hero_entity)
    ```
  """
  @doc group: :relationships
  ...
  ## Examples
    ...
    ```elixir
    :ok = Spearmint.Command.add_parents!([
  ...
  ## Examples
    ...
    ```elixir
    :ok = Spearmint.Command.remove_child!(hero_entity, sword_entity)
    ```
  """
  @doc group: :relationships
  ...
  ## Examples
    ...
    ```elixir
    :ok = Spearmint.Command.remove_children!([
      {hero_entity, [sword_entity]},
      {market_entity, [map_entity, potion_entity]}
    ])
    ```
  ...
  ## Examples
    ...
    ```elixir
    :ok = Spearmint.Command.remove_parent!(sword_entity, hero_entity)
    ```
  """
  @doc group: :relationships
  ...
  ## Examples
    ...
    ```elixir
    :ok = Spearmint.Command.remove_parents!([
      {sword_entity, [hero_entity]},
      {map_entity, [market_entity, vendor_entity]}
    ])
    ```
  ...
  > #### Info  {: .info}
  > An Spearmint instance can only hold one resource of each type at a time.
  ...
  ## Examples
    ...
    ```elixir
    %Demo.Resources.Lobby{} = Spearmint.Command.insert_resource!({Demo.Resources.Lobby, player_count: 0})
    ```
  """
  @doc group: :resources
  ...
  ## Examples
    ...
    ```elixir
    %Demo.Resources.Lobby{} = Spearmint.Command.update_resource!(lobby_resource, player_count: 1)
    ```
  """
  @doc group: :resources
  ...
  ## Examples
    ...
    ```elixir
    %Demo.Resources.Lobby{} = Spearmint.Command.delete_resource!(lobby_resource)
    ```
  """
  @doc group: :resources
  ...
  defp validate_payload(%Operation{name: :insert_resource}, resource_module)
       when is_atom(resource_module), do: :ok
  ...
  defp validate_payload(%Operation{name: :insert_resource} = operation, value),
    do:
      raise(
        Error,
        {operation,
         "Expected  type `Spearmint.Resource.resource_spec()` , got: `#{Kernel.inspect(value)}`"}
      )
  ...
  defp validate_payload(%Operation{name: :delete_resource}, resource) when is_struct(resource),
    do: :ok
  ...
  defp apply_operation(%Operation{name: :spawn_entities} = operation, command, entity_spec_list) do
    entity_spec_list =
      Enum.map(entity_spec_list, fn {_module, opts} ->
        ...
        component_modules =
          Enum.map(component_specs, fn
            ...
            {module, _initial_state} when is_atom(module) -> module
            {module, _initial_state, _tags} when is_atom(module) -> module
          end)
      ...
      Task.async(fn ->
        for entity <- entities do
          ...
          case :ets.lookup(table, {entity.id, Component.Children}) do
            [
              {{_id, Component.Children}, _unknown_field,
               %Component.Children{entities: children_entities}}
            ] ->
      ...
      Task.async(fn ->
        for entity <- entities do
          case :ets.lookup(table, {entity.id, Component.Parents}) do
            [
              {{_id, Component.Parents}, _unknown_field,
               %Component.Parents{entities: parents_entities}}
            ] ->
  ...
  @spec upsert_resource(any(), atom() | {atom(), keyword()}) :: struct()
  @doc false
  ...
  def upsert_resource(operation, {resource_module, state})
  ...
  @spec upsert_components(
          any(),
          any(),
          [atom() | {atom(), list()} | {atom(), list(), maybe_improper_list()}],
          any()
        ) :: any()
  @doc false
  ...
  defp tags_set(component_module, component_spec_tags) do
    ...
    MapSet.new(component_module.__component_tags__())
    |> MapSet.union(MapSet.new(component_spec_tags))
  end
  ...
  defp validate_entities(operation, entities) do
    ...
    case non_entities do
      ...
      _else ->
        raise Error,
              {operation,
               "Expected a list of `Spearmint.Entity.t()` types, got: `#{Kernel.inspect(non_entities)}`"}
    end
  end
  ...
  defp validate_locked_component(_operation, :sync, _component_module) do
    :ok
  end
  ...
  defp validate_component_exists(operation, component) do
    ...
    case :ets.lookup(
      ...
      [{_key, _tags, _val}] -> :ok
      _else -> raise Error, {operation, "#{Kernel.inspect(component)} does not exist"}
    end
  end
  ...
  defp validate_resource_does_not_exist(operation, resource_spec) do
    resource_module =
      case resource_spec do
        {resource_module, _initial_state} when is_atom(resource_module) -> resource_module
  ...
  defp validate_resource_exists(operation, resource) do
    ...
    case :ets.lookup(table, resource.__meta__.module) do
      [{_key, _val}] -> :ok
      _else -> raise Error, {operation, "#{Kernel.inspect(resource)} does not exist"}
    end
  end
  ...
  defp get_component_module(component_spec) do
    case component_spec do
      {component_module, _initial_state, _tags} when is_atom(component_module) -> component_module
      {component_module, _initial_state} when is_atom(component_module) -> component_module
