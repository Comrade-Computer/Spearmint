...
defmodule Spearmint.System do
  @moduledoc """
  The system implements the logic and behaviors of the application
  by manipulating the state of the components.
  The systems are defined by invoking `use Spearmint.System` in their module definition.

  The system modules must implement the
  `c:Spearmint.System.WithoutEventSubscriptions.run/1` or
  `c:Spearmint.System.WithEventSubscriptions.run/2` callbacks,
  depending if the system subscribes to certain events or not.
  The return value of the `run` function is ignored.

  The Spearmint systems run either synchronously or asynchronously,
  as scheduled in the `c:Spearmint.setup/1` callback.
  ...
  - `Spearmint.System.CreateStartupResources` - startup system that creates the default framework resources, states, and custom resources inserted at startup.
  - `Spearmint.System.Timer` - tracks and updates all components using the `Spearmint.Template.Component.Timer` template.

  > #### Info  {: .info}
  > The `Spearmint.Query` and `Spearmint.Command` functions are imported by default
  > for all modules that `use Spearmint.System`
  ...
  The `c:Spearmint.System.WithEventSubscriptions.run/2` callback is triggered
  ...
  However, they are grouped based on their batch keys (see `Spearmint.event/2` options)
  ...
  @doc """
  Utility function. Gives the current process `Spearmint.System` abilities to execute commands.
  ...
  See `Spearmint.TestServer` for more details.
  ...
  def debug(opts \\ []) do
    ...
    system_module = Keyword.get(opts, :system_module, nil)
    ecs_version = Keyword.get(opts, :ecs_version, 0)

    if is_nil(system_module) do
      raise ArgumentError,
            "debug/1 must be called with the :system_module provided to impersonate"
    end

    debug(system_execution, system_module, ecs_version)
  end

  defp debug(system_execution, system_module, ecs_version) do
  ...
  > #### Info  {: .info}
  > This function is already imported for all modules that `use Spearmint.System`
  ...
  ## Examples
    ...
    ```elixir
      Spearmint.System.execute_async(
        enemy_entities,
        fn enemy_entity ->
    ...
    ```
  """
  @spec execute_async(Enumerable.t(), (any() -> any()), keyword()) :: :ok
  ...
  defmodule WithoutEventSubscriptions do
    ...
    It recives the current `t:Spearmint.Frame.t/0` struct as the only argument.
    """
    @callback run(Spearmint.Frame.t()) :: any()
  end
  ...
  defmodule WithEventSubscriptions do
    ...
    and the current `t:Spearmint.Frame.t/0` struct as the second argument.
    """
    @callback run(event :: struct(), Spearmint.Frame.t()) :: any()
  end

  defmacro __using__(opts) do
    quote bind_quoted: [opts: opts], location: :keep do
      import Spearmint.Command
      import Spearmint.Query
      import Spearmint.System, only: [execute_async: 3]
      ...
      case event_modules do
        [] ->
          @behaviour Spearmint.System.WithoutEventSubscriptions

        event_modules when is_list(event_modules) ->
          Spearmint.Util.validate_events(event_modules)
          @behaviour Spearmint.System.WithEventSubscriptions
        ...
        event_modules ->
          ...
          def schedule_run(frame) do
            Enum.each(frame.event_batches, fn events ->
              ...
              |> Spearmint.System.execute_async(&run(&1, frame), concurrent: length(events) + 1)
            end)
          end
      end
